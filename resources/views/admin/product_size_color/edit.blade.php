...
<div class="container">
    ...
    <form action="{{ route('admin.product_size_color.update', $item->id) }}" method="POST">
        ...
        <div class="mb-3">
            ...
            <input type="text" name="import_price" id="import_price" class="form-control" value="{{ $item->import_price ?? '' }}" required>
        </div>

        <div class="mb-3">
            <label for="regular_price" class="form-label">Giá định bán</label>
            <input type="text" name="regular_price" id="regular_price" class="form-control" value="{{ $item->regular_price ?? '' }}" required>
        </div>
